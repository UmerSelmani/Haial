/* ═══════════════════════════════════════
   HAIAL — Streams Page (Live RSS Feeds)
   Table 1: Islamic Sources
   Table 2: Coin News (all sources)
   ═══════════════════════════════════════ */
...
function renderNewsItem(item) {
  const coinTags = item.coins.length
    ? item.coins.map(coinBadge).join(' ')
    : '';
  ...
  return '<a href="' + item.link + '" target="_blank" rel="noopener" class="stream-item glow-card" style="text-decoration:none;display:block">' +
    ...
    '</div>' +
    '<h4 style="font-size:13px;font-weight:600;color:var(--text-primary);line-height:1.4;margin-bottom:6px">' + item.title + '</h4>' +
...
function renderCoinFilter() {
  ...
  return '<div class="stream-filter" style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:12px">' +
    '<button class="stream-filter-btn active" onclick="filterCoinNews(\'all\')" data-coin="all">All</button>' +
    coins.map(c =>
...
async function loadIslamicFeeds(force) {
  ...
  el.innerHTML = items.slice(0, 20).map(renderNewsItem).join('');
}
...
function filterCoinNews(ticker) {
  ...
  if (filtered.length === 0) {
    el.innerHTML = '<div class="text-center" style="padding:24px;color:var(--text-dimmed)">' +
      '<p style="font-size:12px">' + (ticker === 'all' ? 'Could not load feeds.' : 'No recent news for ' + ticker) + '</p></div>';
    return;
  }
